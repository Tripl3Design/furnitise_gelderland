"use strict"
function addColors(name, colorName, containerElem) {
    const html = [];
    ...
    for (let c = 0; c < textureItems.length; c++) {
       ...
       // }
        ...
        if (name == 'outsideColors' || name == 'colors' || name == 'bodyColor' || name == 'selectedComponentColor') {
            ...
            //if (name == 'layerColor' || name == 'selectedComponentColor') {
            html.push( /*html*/ `
                <div class="col d-flex align-items-center m-0 p-1" style="aspect-ratio: 1">
                <!-- id index + 30 is HACK -->
                    <img id="${name}Index_${c + 30}" style="background-color: #${textureItems[c].colorHex}; width: 75px;" src="${textureItems[c].colorPathThumb}" class="rounded-pill img-fluid mx-auto p-0 border    border-5   colorButton ${name}_colorButton" alt="${textureItems[c].colorName}">
                </div>
            `);
        } else {
            html.push( /*html*/ `
                <div class="col d-flex align-items-center m-0 p-1" style="aspect-ratio: 1" data-bs-toggle="tooltip" data-bs-title="${textureItems[c].colorName}">
                    <img id="${name}Index_${c}" style="background-color: #${textureItems[c].colorHex}; width: 75px;" src="${textureItems[c].colorPathThumb}" class="rounded-pill img-fluid mx-auto p-0 border    border-5    colorButton ${name}_colorButton" alt="${textureItems[c].colorName}">
                </div>
            `);
        }
    }
...
function controlPanel_updateLayer(name, settings) {
    ...
    `);
    html.push( /*html*/ `
        <div class="col-xxl-4 col-xl-4 col-12 h6 fw-bold text-uppercase m-0 p-0 py-xxl-4 py-xl-4 pt-3 pb-1">${accordion.title}</div>
        `);
    html.push( /*html*/ `
            <div id="${name}Text" class="col-xxl-4 col-xl-4 col-12 h6 fw-normal m-0 p-0 my-xxl-auto my-xl-auto mb-3 accordion-text fadein align-items-center">
            `);
...
function controlPanel_addLayer(name, settings, collapsed) {
    ...
    /*html*/ ` 
        <div id="show${name[0].toUpperCase()}${name.slice(1)}" class="accordion-item m-0 p-0 shadow-none border border-0 border-top border-dark ${accordion.display}">
            <div id="collapse-check-${name}" class="m-0 p-0      shadow-none accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#body-${name}" aria-expanded="${!collapsed}" aria-controls="${name}" onclick="showSelected(${accordion.displayTitle});"></div>
