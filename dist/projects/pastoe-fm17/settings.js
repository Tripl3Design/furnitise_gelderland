...
function updateFeaturedModel(model) {
    UNITY_INSTANCE.SendMessage('Fm17', 'SetFm17', JSON.stringify(model));
}
...
function updateControlPanel(model, selectedLayer, expandedLayer) {
    ...
    if (model.fabricColor != undefined) {
        ...
        if (model.armrestColor != undefined) {
            const armrestColor = model.armrestColor;
            var armrestColorIndex = ALLCOLORS.armrestColors.findIndex((item) => item.colorPath === armrestColor);
            ...
            document.getElementById('armrestColorsText').innerHTML = '<img src="https://' + window.location.host + '/' + ALLCOLORS.armrestColors[armrestColorIndex].colorPathThumb + '" class="rounded-pill shadow" style="width: calc(1rem + 1vw);">';
            document.getElementById('armrestColorsIndex_' + armrestColorIndex).classList.remove('colorButton');
            document.getElementById('armrestColorsIndex_' + armrestColorIndex).classList.add('colorButtonActive');
        }

        //price
        document.getElementById('price').textContent = "EURO 695,-";
    }
    ...
    function initSettings(model) {
        ...
        <div class="row m-0 p-0 pb-2">
            ...
            "onload": function () {
                let containerElem = document.getElementById("armrestColorsTexturePicker");
                addTextures('armrestColors', ALLCOLORS.armrestColors, containerElem);
            }
        }
        return { accordions };
    }
